...
<template>
  <AppLayout><div class="container-fluid py-3">
    ...
    <template v-else>
      ...
      <div v-if="dashboard" class="row g-4 mt-1">
        <section class="col-lg-6"><div class="card h-100 shadow-sm"><div class="card-body"><h5>Placement drives by status</h5><div v-if="!driveStatuses.length" class="text-muted">No placement drive data yet.</div><div v-for="item in driveStatuses" :key="item.label" class="mb-3"><div class="d-flex justify-content-between small mb-1"><span>{{ formatEnum(item.label) }}</span><strong>{{ item.count }}</strong></div><div class="progress" role="img" :aria-label="`${formatEnum(item.label)}: ${item.count} drives`"><div class="progress-bar" :style="{ width: item.width }"></div></div></div></div></div></section>
        <section class="col-lg-6"><div class="card h-100 shadow-sm"><div class="card-body"><h5>Accounts by status</h5><div v-if="!accountStatuses.length" class="text-muted">No account data yet.</div><div v-for="item in accountStatuses" :key="item.label" class="mb-3"><div class="d-flex justify-content-between small mb-1"><span>{{ formatEnum(item.label) }}</span><strong>{{ item.count }}</strong></div><div class="progress" role="img" :aria-label="`${formatEnum(item.label)}: ${item.count} accounts`"><div class="progress-bar bg-success" :style="{ width: item.width }"></div></div></div></div></div></section>
      </div>
    </template>
  </div></AppLayout>
</template>
